...
struct PANOC_iterable{R<:Real,C<:Union{R,Complex{R}},Tx<:AbstractArray{C},Tf,TA,Tg,Tpg,TH}
    f::Tf             # smooth term
...
mutable struct PANOC_state{R<:Real,Tx,TAx,TH}
    ...
    tau::R     # stepsize (can be nothing since the initial state doesn't have it)
...
function Base.iterate(iter::PANOC_iterable{R}) where {R}
    ...
    if gamma === nothing
        ...
        xeps = x .+ R(1)
        f_Axeps, grad_f_Axeps = iter.f(iter.A * xeps)
...
function Base.iterate(
    ...
    state.f_Ax_d, state.grad_f_Ax_d = iter.f(state.Ax_d)
    ...
    for i = 1:20
        ...
        if false #ProximalOperators.is_quadratic(iter.f)
            ...
            state.f_Ax, state.grad_f_Ax = iter.f(state.Ax)
