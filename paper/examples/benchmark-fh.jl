...
function print_config(CFG)
    ...
    println("  OPNORM_MAXITER  = $(CFG.OPNORM_MAXITER)")
    println("  HESSIAN_SCALE  = $(CFG.HESSIAN_SCALE)")
end
...
function run_tr!(model, x0; λ = 1.0, qn = :LSR1, atol = 1e-3, rtol = 1e-3, verbose = 0, sub_kwargs = (;), opnorm_maxiter = 4, scaling_factor = 1.0)
    qn_model = ensure_qn(model, qn)
    reset!(qn_model)  # reset des compteurs
    qn_model.op.data.scaling_factor = scaling_factor
...
# ======== R2N run ======== #
#############################
function run_r2n!(model, x0; λ = 1.0, qn = :LBFGS, atol = 1e-3, rtol = 1e-3, verbose = 0, sub_kwargs = (;), σk = 1e5, opnorm_maxiter = 4)
    qn_model = ensure_qn(model, qn)
    reset!(qn_model)
    reg_nlp  = RegularizedNLPModel(qn_model, NormL0(λ))
...
if :TR in CFG.RUN_SOLVERS
    push!(results, run_tr!(model, x0; λ = CFG.LAMBDA_L0, qn = CFG.QN_FOR_R2N, atol = CFG.TOL, rtol = CFG.RTOL, verbose = CFG.VERBOSE_RO, sub_kwargs = CFG.SUB_KWARGS_R2N, opnorm_maxiter = CFG.OPNORM_MAXITER, scaling_factor = CFG.HESSIAN_SCALE))  # test LBFGS aussi
end
if :R2N in CFG.RUN_SOLVERS
    push!(results, run_r2n!(model, x0; λ = CFG.LAMBDA_L0, qn = CFG.QN_FOR_R2N, atol = CFG.TOL, rtol = CFG.RTOL,
                            verbose = CFG.VERBOSE_RO, sub_kwargs = CFG.SUB_KWARGS_R2N, σk = CFG.SIGMAK_R2N, opnorm_maxiter = CFG.OPNORM_MAXITER))
end
