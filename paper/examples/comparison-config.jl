...
Base.@kwdef mutable struct Config
    ...
    M_MONOTONE::Int           = 1  # for nonmonotone R2N
end

# One global, constant *binding* to a mutable object = type stable & editable
const CFG = Config(QN_FOR_TR = :LBFGS)
const CFG2 = Config(SIGMAK_R2N=eps()^(1 / 5), TOL = 1e-4, RTOL = 1e-4, QN_FOR_R2N=:LSR1)
const CFG3 = Config(SIGMAK_R2N=eps()^(1 / 5), TOL = 1e-4, RTOL = 1e-4, RUN_SOLVERS = [:LM, :TR, :R2N], QN_FOR_TR = :LBFGS)
