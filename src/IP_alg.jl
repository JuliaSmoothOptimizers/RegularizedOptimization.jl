...
function IntPt_TR(
	...
	while kktNorm[1] > ϵD && k < maxIter
		...
		if norm(Gν)>ϵD #final stopping criteria 
			...
			(s, s⁻, hist, funEvals) = s_alg(objInner, (d)->ψk(xk + d), s, (d, λν)->Rkprox(d, λν, xk, min(10*norm(s,2), Δk)	), FO_options) #2*||s_1|| = Δk (same norm as TR) min(ξ*||s_1||, Δk)min(10*norm(s,2), Δk)
			Gν = s/FO_options.ν
		else
			funEvals = 1 
		end
