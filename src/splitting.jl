...
function prox_split_1w(proxp, s0, projq, options)
  ...
  for i = 1:restart
    while err > ε && k < max_iter && w_err > ε_w
      ...
      k % print_freq == 0 && @printf(
        "Iter: %4d, ||w-x-s||^2: %1.5e, s-err: %1.5e, w-err: %1.5e η: %1.5e, ξ: %1.5e \n",
        k,
        w_err,
        norm(s_ - s),
        norm(w_ - w),
...
function prox_split_2w(proxp, s0, projq, options)
  ...
  for i = 1:restart
    ...
    while err > ε && k < max_iter #&& (w1_err+w2_err)>ε_w
      ...
      k % print_freq == 0 && @printf(
        "iter: %d, ||w1-u||²: %7.3e, ||w2-u+xk||²: %7.3e, err: %7.3e, η: %7.3e, s_feas: %7.3e, ||w1||_1: %7.3e, ||w2||_1: %7.3e, u-sum: %7.3e\n",
        k,
        w1_err,
        w2_err,
        err,
