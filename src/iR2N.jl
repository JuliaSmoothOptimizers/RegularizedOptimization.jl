...
function SolverCore.solve!(
  ...
  if verbose > 0
    @info log_header(
      [:outer, :inner, :fx, :hx, :norm_s1_νInv, :ρ, :σ, :normx, :norms, :normB, :arrow],
  ...
  norm_s1_νInv = norm(s1) / ν₁
  # solved = (ξ1 < 0 && sqrt_ξ1_νInv ≤ neg_tol) || (ξ1 ≥ 0 && sqrt_ξ1_νInv ≤ atol * (√κs))
  solved = norm_s1_νInv ≤ atol * κs # new stopping test according to theory
  (ξ1 < 0 && sqrt_ξ1_νInv > neg_tol) &&
    @warn "iR2N: prox-gradient step should produce a decrease but ξ1 = $(ξ1) and sqrt_ξ1_νInv = $sqrt_ξ1_νInv > $neg_tol"
  atol += rtol * norm_s1_νInv # make stopping test absolute and relative
  ...
  while !done
    sub_atol = stats.iter == 0 ? 1.0e-3 : min(norm_s1_νInv^(1.5), norm_s1_νInv * 1e-3)
    ...
    (ξ1 < 0 && sqrt_ξ1_νInv > neg_tol) &&
      @warn "iR2N: prox-gradient step should produce a decrease but ξ1 = $(ξ1) with sqrt_ξ1_νInv = $sqrt_ξ1_νInv > $neg_tol"
    set_status!(
      stats,
      get_status(
        reg_nlp,
  ...
  if verbose > 0 && stats.status == :first_order
    ...
    @info "iR2N: terminating with ‖s‖/ν = $(norm_s1_νInv)"
  end
  ...
  set_residuals!(stats, zero(eltype(xk)), norm_s1_νInv)
  return stats
end