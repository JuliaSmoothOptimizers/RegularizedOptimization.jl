...
function LMTR(
  ...
  while !(optimal || tired)
    ...
    if η2 ≤ ρk < Inf
      Δk = max(Δk, γ * sNorm)
      !(has_bounds(nls) || subsolver == TRDH) && set_radius!(ψ, Δk)
    end

    if η1 ≤ ρk < Inf
      xk .= xkn
      (has_bounds(nls) || subsolver == TRDH) && set_bounds!(ψ, max.(-Δk, l_bound - xk), min.(Δk, u_bound - xk))
