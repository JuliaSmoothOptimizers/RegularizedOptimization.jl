...
function SolverCore.solve!(
  ...
  if verbose > 0
    @info log_header(
      [:iter, :fx, :hx, :norm_s_νInv, :ρ, :σ, :normx, :norms, :dualgap, :arrow],
  ...
  norm_s_νInv = norm(s) / ν
  atol += rtol * norm_s_νInv # make stopping test absolute and relative

  # solved = (ξ < 0 && sqrt_ξ_νInv ≤ neg_tol) || (ξ ≥ 0 && sqrt_ξ_νInv ≤ atol * (√κs))
  solved = norm_s_νInv ≤ atol * κs # new stopping test according to theory
  (ξ < 0 && sqrt_ξ_νInv > neg_tol) &&
    @warn "iR2: first prox-gradient step should produce a decrease but ξ = $(ξ) and sqrt_ξ_νInv = $(sqrt_ξ_νInv) > $(neg_tol)"

  set_solver_specific!(stats, :norm_s_νInv, norm_s_νInv)
  set_status!(
    stats,
    get_status(
      reg_nlp,
  ...
  if done
    println("iR2: terminating with ‖s‖/ν = $(norm_s_νInv) at iteration $(stats.iter)")
  end
  ...
  while !done
    ...
    solved = norm_s_νInv ≤ atol * κs # new stopping test according to theory
    (ξ < 0 && sqrt_ξ_νInv > neg_tol) &&
      @warn "iR2: prox-gradient step should produce a decrease but ξ = $(ξ) and sqrt_ξ_νInv = $(sqrt_ξ_νInv) > $(neg_tol)"
  ...
  if verbose > 0 && stats.status == :first_order
    ...
    @info "iR2: terminating with ‖s‖/ν = $(norm_s_νInv)"
  end
  ...
  set_residuals!(stats, zero(eltype(xk)), norm_s_νInv)
  return stats
end