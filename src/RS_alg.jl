...
function ReSp1(
  ...
  xk = similar(x0) #maybe should be zero?
  hk = h(xk)
  if hk == Inf
  ...
  z = zero(xk)
  ψ = shifted(h, xk)
  JtJ = Jk'*Jk #you can be smarter here
  JtF = Jk'*Fk #and here
  ...
  while !(optimal || tired)
    ...
    @show fkhk, fk+hk, ξ, ξ1, ν, norm(z - xk)
...
function  ReSp2(
  ...
  JtJ,
  JtF
  ) where {F <: Function, G <: Function, H <: Function, C <: Function}
  ϵ = options.ϵ
  verbose = options.verbose
  maxIter = options.maxIter
