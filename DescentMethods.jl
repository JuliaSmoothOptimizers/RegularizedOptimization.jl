...
function s_options(β;optTol=1f-5, maxIter=10000, verbose=2, restart=10, λ=1.0, η =1.0, η_factor=.9, 
     WoptTol=1f-5, gk = Vector{Float64}(undef,0), Bk = Array{Float64}(undef, 0,0), xk=Vector{Float64}(undef,0))

	return s_params(optTol, maxIter, verbose, restart, β,λ, η, η_factor, WoptTol, gk, Bk,xk)
...
#sj,σ,ν, p_norm, q_norm, params,
    ...
    while err>ε && k<max_iter && (w1_err+w2_err)>ε_w
        ...
        w1_err = norm(w1 - u)^2
        w2_err = norm(w2 - u + xk)^2
        err = norm(s_ - s) + norm(w1_ - w1) + norm(w2_ - w2)
        k % print_freq ==0 && 
        @printf("iter: %d, ||w1-u||²: %7.3e, ||w2-u+xk||²: %7.3e, err: %7.3e, η: %7.3e \n", k, w1_err, w2_err, err, η)
