using Random, LinearAlgebra, TRNC, Printf,Roots, LinearOperators, ShiftedProximalOperators
...
function L0Binf(compound = 1)
    ...
    parameters = TRNCmethods(ϕ, Ψ; FO_options = Doptions, s_alg=PGnew, χk=(s)->norm(s, Inf))
    ...
    Ψ = ShiftedNormL0(n, λ)
    parametersQR = TRNCmethods(ϕ, Ψ; FO_options = Doptions, s_alg=PG, χk=(s)->norm(s, Inf))
    optionsQR = TRNCparams(; σk = 1/β, ϵ=ε, verbose = 10) #options, such as printing (same as above), tolerance, γ, σ, τ, w/e

    #input initial guess
    xqr, kqr, Fhistqr, Hhistqr, Comp_pgqr = QRalg(xi, parametersQR, optionsQR)
