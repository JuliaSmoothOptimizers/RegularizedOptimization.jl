...
function demo_fh()
  ...
  h = NormL0(1.0)
  χ = NormLinf(1.0)
  options = ROSolverOptions(; verbose = 10, maxIter = 200, ϵ = 1e-6, β = 1e16, ν = 1.0e+2)
  sub_options = ROSolverOptions(maxIter = 100_000)

  lbfgs_model = LBFGSModel(model)
  TR_out = TR(lbfgs_model, h, χ, options, subsolver_options = sub_options)
  # plot_fh(TR_out, simulate(TR_out.solution), data, "tr-r2")

  nls_model = ADNLSModel(resid, ones(5), 202)
  LMTR_out = LMTR(nls_model, h, χ, options, subsolver_options = sub_options)
  # plot_fh(LMTR_out, simulate(LMTR_out.solution), data, "lmtr-r2")

  reset!(nls_model)
  LM_out = LM(nls_model, h, options, subsolver_options = sub_options)
